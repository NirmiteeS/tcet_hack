...
def schedule_meeting(text, from_email):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error creating datetime object: {e}")
            return "Error processing date and time. No email sent."
        
        creds = Credentials.from_authorized_user_file('token.json', SCOPES)
        conflicts = check_calendar_conflicts(creds, proposed_start, proposed_end)
        
        if conflicts:
            return "Conflicts detected. Please choose an alternative time."
        ...
        return f"Meeting '{title}' scheduled on {date} at {time} with attendees: {list(valid_attendees)}."
    except Exception as e:
        logger.error(f"Error scheduling meeting: {e}")
        return "Error scheduling meeting. No email sent."
...
def analyze_email_for_tasks(email_body: str, msg_id: str) -> List[dict]:
    """Analyze email content using Gemini to extract tasks."""
    try:
        prompt = f"""
